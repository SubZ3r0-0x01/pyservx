...
# Enhanced Version 2.0.0 with Advanced Features
...
PORT = 8088
CONFIG_FILE = os.path.expanduser("~/.pyservx_enhanced_config.json")
...
def get_shared_folder():
    ...
    # Create PyServeX Enhanced shared folder in Downloads
    shared_folder = os.path.join(downloads_dir, "PyServeX-Enhanced-Shared")
    
    try:
        if not os.path.exists(shared_folder):
            os.makedirs(shared_folder)
            print(f"Created enhanced shared folder: {shared_folder}")
        else:
            print(f"Using existing enhanced shared folder: {shared_folder}")
        ...
        fallback_folder = os.path.join(os.getcwd(), "shared-enhanced")
        try:
...
def run(base_dir, no_qr=False, port=None):
    """Run the enhanced HTTP server with the specified base directory."""
    global PORT
    if port:
        PORT = port
    ...
    class Handler(request_handler.EnhancedFileRequestHandler):
    ...
    if not no_qr:
        ...
        print("PyServeX Enhanced - System IPv4 addresses:")
    ...
    try:
        ...
        print(f"PyServeX Enhanced v2.0.0 serving at http://0.0.0.0:{PORT}")
        print("Enhanced features: Analytics, Thumbnails, Advanced Search, File Operations")
        
        def shutdown_handler(signum, frame):
            print("\\nShutting down PyServeX Enhanced...")
            if server:
...
def main():
    """Main entry point for the enhanced command-line tool."""
    parser = argparse.ArgumentParser(description="PyServeX Enhanced: Advanced HTTP server for file sharing with analytics and enhanced features.")
    parser.add_argument('--version', action='version', version='PyServeX Enhanced 2.0.0')
